...
update_system() {
    ...
    case "$pkg_manager" in
        "apt")
            $install_cmd curl wget jq git docker.io docker-compose iputils-ping >/dev/null 2>&1
            ;;
        "yum"|"dnf")
            $install_cmd curl wget jq git docker docker-compose iputils-ping >/dev/null 2>&1
            ;;
        "pacman")
            $install_cmd curl wget jq git docker docker-compose iputils-ping >/dev/null 2>&1
            ;;
